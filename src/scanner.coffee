...
class @Scanner
  ...
  constructor: (@settings) ->
    ...
    # Has the USL header been found?
    @uslHeaderFound = !@settings.validate
    ...
    while not found
      ...
      if @peek() == '#'
        while @peek() not in C_LB + '\x00'
          unless @uslHeaderFound
            comment += @peek()
          @forward()

      # the first bytes in a USL file MUST be the USL version
      unless @uslHeaderFound
        trimmedComment = comment.trim()
        if trimmedComment and USL_VERSION_RE.test(trimmedComment)
          if trimmedComment is USL_VERSION
            @uslHeaderFound = true
          else
            throw new exports.ScannerError 'version validation', null, "Unsupported USL version: '#{comment}'", @create_mark(0, 0)
        else
          throw new exports.ScannerError 'version validation', null, "The first line must be: '#{USL_VERSION}'", @create_mark(0, 0)
